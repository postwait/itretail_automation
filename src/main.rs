...
fn main() {
    let mut cmd = Command::new("itretail_automation")
        ...
        .subcommand(Command::new("scale-export")
            ...
            .arg(Arg::new("scale").long("scale").conflicts_with("no-scales").action(ArgAction::Append).value_name("IP Address"))
            .arg(Arg::new("no-scales").long("no-scales").short('n').conflicts_with("scale").action(ArgAction::SetTrue).num_args(0))
            ...
            .arg(Arg::new("at-least").long("at-least").short('q').action(ArgAction::Set).value_name("weight/qty").value_parser(clap::value_parser!(f32)).default_value("-10000000.0"))
